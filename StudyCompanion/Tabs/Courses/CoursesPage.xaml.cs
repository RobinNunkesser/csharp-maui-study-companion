...
public partial class CoursesPage : ContentPage
{
    ...
    {
        await DisplayAlert(AppResources.Error, AppResources.IOSPermissions,
            AppResources.OK);
    }

    private async Task DisplayPermissionRationale()
    {
        await DisplayAlert(AppResources.Error, AppResources.PermissionRationale,
            AppResources.OK);
    }
    ...
    {
        var status =
            await Permissions.CheckStatusAsync<Permissions.CalendarRead>();
    ...
    private async Task<PermissionStatus>
        CheckAndRequestCalendarWritePermission()
    {
        var status =
            await Permissions.CheckStatusAsync<Permissions.CalendarWrite>();
    ...
    private async void OnCollectionViewSelectionChanged(object sender,
        SelectionChangedEventArgs e)
    {
        ...
        var editableCalendars =
            calendars.Where(c => c.CanEditCalendar).ToList();

        var selectedCalendar = editableCalendars.FirstOrDefault();

        if (editableCalendars.Count == 0)
        {
            await DisplayAlert(AppResources.Error, AppResources.NoCalendars,
                AppResources.OK);
            return;
        }

        var names = editableCalendars.Select(c => c.Name).ToArray();
        var chosenCalendar = await DisplayActionSheet(
            AppResources.CalendarExportQuery, AppResources.Cancel, null, names);
        if (chosenCalendar.Equals(AppResources.Cancel)) return;
        ...
        _viewModel.AddCourseToCalendar(
            e.CurrentSelection.FirstOrDefault() as ICourse, selectedCalendar);
    }
